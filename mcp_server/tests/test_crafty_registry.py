"""Tests for infra/crafty_registry.py - real tmp_path SQLite, no mocking.
...
def test_register_then_get_round_trips_every_field(tmp_path: Path):
    ...
    fetched = crafty_registry.get(db, "survival")
...
def test_registering_the_same_name_again_overwrites_the_old_entry(tmp_path: Path):
    ...
    assert len(crafty_registry.list_all(db)) == 1


def test_unknown_world_name_lists_the_names_that_exist(tmp_path: Path):
    db = tmp_path / "worlds.db"
    crafty_registry.register(
        db, "survival", server_id="a", api_token="t", base_url="https://x", verify_ssl=True
    )
    crafty_registry.register(
        db, "creative", server_id="b", api_token="t", base_url="https://x", verify_ssl=True
    )

    with pytest.raises(KeyError) as error:
        crafty_registry.get(db, "skyblock")
...
def test_remove_deletes_a_registered_world(tmp_path: Path):
    ...
    crafty_registry.remove(db, "survival")

    assert crafty_registry.list_all(db) == []
...
def test_remove_leaves_other_worlds_untouched(tmp_path: Path):
    ...
    names = [world.name for world in crafty_registry.list_all(db)]
...
def test_removing_an_unregistered_name_lists_the_names_that_exist(tmp_path: Path):
    ...
    with pytest.raises(KeyError) as error:
        crafty_registry.remove(db, "skyblock")
...
def test_removing_from_an_empty_registry_says_so(tmp_path: Path):
    ...
    with pytest.raises(KeyError, match="No worlds"):
        crafty_registry.remove(db, "anything")
...
def test_no_worlds_registered_at_all_says_so(tmp_path: Path):
    ...
    with pytest.raises(KeyError, match="No worlds"):
        crafty_registry.get(db, "anything")
...
def test_get_default_is_none_when_nothing_has_been_set(tmp_path: Path):
    ...
    assert crafty_registry.get_default(db) is None


def test_set_default_then_get_default_round_trips(tmp_path: Path):
    db = tmp_path / "worlds.db"

    set_result = crafty_registry.set_default(db, base_url="https://crafty.example.com", verify_ssl=False)
    fetched = crafty_registry.get_default(db)
...
def test_setting_the_default_again_overwrites_the_old_value(tmp_path: Path):
    db = tmp_path / "worlds.db"
    crafty_registry.set_default(db, base_url="https://old.example.com", verify_ssl=True)

    crafty_registry.set_default(db, base_url="https://new.example.com", verify_ssl=False)
