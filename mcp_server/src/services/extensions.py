...
/capabilities extends to proxied
...
# ordinary word separators, so a single underscore here couldn't be told
# apart from part of the extension id or the upstream tool's own name.
# Double underscore is reserved specifically to mark this namespace
# boundary.
...
# startup indefinitely - see ExtensionRegistry._connect_one. Also set as
# the connected ClientSession's default read_timeout_seconds, which is
# why ExtensionRegistry.call() below must override it per-call with
# CALL_TIMEOUT_SECONDS instead of inheriting this connect-phase value.
CONNECT_TIMEOUT_SECONDS = 10.0

# How long a single proxied tool call may run before it's treated as
# failed. Deliberately separate from CONNECT_TIMEOUT_SECONDS: a proxied
# tool can do real upstream work (e.g. a backend call) that
# routinely exceeds the few seconds a connect handshake should ever
# take, so reusing that shorter budget here would time out legitimate
# slow tools, not just hung ones.
...
class ExtensionRegistry:
    ...
    A class rather than bare module globals (contrast a module-level registry of
    handlers registered once at import time and never torn down): this one holds open subprocesses and network-shaped resources that a
    ...
    async def merged_list_tools(self) -> list[types.Tool]:
        ...
        A tool registered with meta={"hidden": True} (e.g.
        tool_xxx_getAuditLog) is dropped from this listing - so it
        never appears to the AI agent or chat_app's command/tool UI - but
        stays fully callable via merged_call_tool()/call_tool() for a
        caller that already knows its name, like chat_app's Logs page.
        """
        assert self._mcp is not None, "install() must run before merged_list_tools()"
        own_tools = await self._mcp.list_tools()
        visible_own_tools = [t for t in own_tools if not (t.meta or {}).get("hidden")]
        return [*visible_own_tools, *self.proxied_tool_definitions()]
    ...
    async def merged_call_tool(self, name: str, arguments: dict[str, Any]) -> Any:
        ...
        split /capabilities relies on, since it did not need to change to cover proxied tools; they just see
