...
async def test_stats_gets_the_servers_stats_endpoint():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        result = await crafty_client.stats("https://crafty.example.com", "tok3n", "srv1")
...
async def test_authorization_header_carries_the_api_token():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        await crafty_client.stats("https://crafty.example.com", "tok3n", "srv1")
...
async def test_action_posts_to_the_action_endpoint():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        await crafty_client.action("https://crafty.example.com", "tok3n", "srv1", "start_server")
...
async def test_unsupported_action_is_rejected_before_any_request_is_made():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        with pytest.raises(ValueError, match="kill_server"):
            await crafty_client.action("https://crafty.example.com", "tok3n", "srv1", "kill_server")
...
async def test_send_command_posts_plain_text_to_stdin():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        await crafty_client.send_command("https://crafty.example.com", "tok3n", "srv1", "say hello")
...
async def test_a_non_ok_status_field_raises_crafty_error_with_the_message():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        with pytest.raises(crafty_client.CraftyError, match="Server not found"):
            await crafty_client.stats("https://crafty.example.com", "tok3n", "srv1")
...
async def test_a_connection_failure_raises_a_recoverable_crafty_error():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        with pytest.raises(crafty_client.CraftyError, match="Could not reach Crafty"):
            await crafty_client.stats("https://crafty.example.com", "tok3n", "srv1")
...
async def test_ping_reports_reachable_for_a_200_response():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        result = await crafty_client.ping("https://crafty.example.com")
...
async def test_ping_strips_a_trailing_slash_from_base_url():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        await crafty_client.ping("https://crafty.example.com/")
...
async def test_ping_reports_unreachable_on_timeout():
    ...
    with patch("src.crafty_client.aiohttp.ClientSession", session_cls):
        result = await crafty_client.ping("https://crafty.example.com", timeout=1.0)
