...
description: Add a new AI agent persona/role, a new running ai_agent instance (for chat_app's Agent dropdown / delegate_to_agent), or a brand-new LLM provider backend to the ai_agent project. Use whenever the user asks to add an AI agent, a new persona/role for the assistant, a new provider or model family (beyond Codex/OpenAI), a new gateway (OpenRouter/Bedrock/Vertex/Azure/etc), or wants another agent instance available for delegation — even if they only describe the desired behavior ("I want an agent that specializes in X", "hook up a Gemini-backed agent") without naming ai_agent or these files explicitly.
---
...
| Support for a model family that isn't Codex or OpenAI (Gemini, a bare Ollama/vLLM box not reachable through the OpenAI-compatible shim, etc.) | **Provider** | Yes |
...
4. **Know the limitation before promising a customer two personas on
   ...
   registered id from its *provider* alone (`Codex-agent`,
